...
test('clears the inherited inbox at session start and logs the drop', () => {
  ...
  harness.emit('agent/session-start', { agent: forked, source: 'startup' })
...
test('leaves the inbox alone for every other start', () => {
  ...
  harness.emit('agent/session-start', { agent: resumed, source: 'resume' })
  const fresh = agent({ seeded: false, nextTurn: [{}] })
  harness.emit('agent/session-start', { agent: fresh, source: 'startup' })
  const empty = agent({ seeded: true })
  harness.emit('agent/session-start', { agent: empty, source: 'startup' })
