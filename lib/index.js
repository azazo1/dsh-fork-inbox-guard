...
export function apply(ctx) {
    ctx.on('agent/session-start', (payload) => {
        if (!inheritedPendingInput(payload.agent, payload.source))
            return;
        payload.agent.inbox.clear();
        ctx.logger.info(`fork-inbox-guard: dropped the pending inbox of seeded session "${payload.agent.id}"`);
    });
}
/**
 ...
 */
export function inheritedPendingInput(agent, source) {
    if (source !== 'startup')
        return false;
    if (!agent.session.header.isSeeded)
        return false;
